...
const stockValue = computed(() =>
  (props.current * props.costPerUnit).toLocaleString()
)
...
<template>
  ...
  >
    ...
    <div class="flex justify-between items-start mb-2">
      <div>
        <span class="font-semibold text-[#1A1A2E] text-sm block">
          {{ name }}
        </span>
      ...
      <span class="text-lg font-bold text-[#1A1A2E]">
        {{ current }} {{ unit }}
      </span>
    </div>

    <!-- Bar -->
    <div class="w-full h-2 bg-gray-100 rounded-full overflow-hidden relative my-2">
      <div
    ...
    <div class="flex justify-between text-xs text-gray-400 mt-1">
      ...
      <span v-if="costPerUnit > 0">
        Value: KES {{ stockValue }}
      </span>
    </div>

    <!-- Alert -->
    <p
      v-if="current < reorder && current > 0"
